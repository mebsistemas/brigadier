...
fmdescuentos.showmodal;
 ...
 resultado:=true;
  ...
  if self.queryvece.RecordCount > 0 then
   begin
      if Application.MessageBox( pchar('El artículo '+trim(queryvece.Fields[2].asstring)
              +' Lote Nro: '+trim(queryvece.Fields[0].asstring)+' está vencido. (vence: '+trim(queryvece.Fields[1].asstring)+'). ¿Desea continuar?'), 'ATENCION. ARTICULO VENCIDO',
...
begin
  showmessage('Debe abrir la caja del día');
  exit;
end;
...
if fdvencimientos.RecordCount > 0  then
 begin
   Application.MessageBox( 'ATENCION. TIENE PRODUCTOS POR VENCER EN EL DÍA DE HOY.',
...
lABEL2.Caption:='00.00';
   ...
   FDQuery2.SQL.Add('SELECT * FROM TCAJA WHERE estado=1 order by idcaja desc');
   FDQuery2.Open;
   PANEL2.Caption:='CAJA '+ FDQuery2.FieldByName('fecha').AsString;
...
BEGIN
 ...
 SIVA21:=0;
  ...
  FRMIVAVENTA.QRLabel2.Caption:='MES Y AÑO : '+TRIM(MESYANIO.ComboBox1.Text)+'  '+TRIM(MESYANIO.Edit1.Text);
...
' (SUM(TM.TOTAL)-SUM(TM.PAGO)) AS SALDO  '+
' FROM TMOVIMIENTOS  TM,TCLIENTES TC  '+
' WHERE    '+
 ' TM.IDFORMAPAGO=5  '+
' AND TM.IDCLIENTE=TC.IDCLIENTE  '+
...
' HAVING   (SUM(TM.TOTAL)-SUM(TM.PAGO)) > 0 '+
...
frmterminar.Edit1.Text:=TRIM(LABEL2.Caption);
